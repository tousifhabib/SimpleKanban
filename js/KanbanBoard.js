...
export default class KanbanBoard {
    constructor() {
        this.state = loadFromLocalStorage(STORAGE_KEY) || { columns: [] };
    ...
    // Create a column element from its data
    ...
    // Create a card element from its data
    ...
    // Set up event listeners for a column
    ...
    // Business logic: add a new column or card
    addNewColumn(title) {
        this.state.columns.push({ id: generateId('column'), title: title || 'New Column', cards: [] });
        this.saveState();
        this.renderBoard();
    }
    ...
    // Update state order based on the DOM order of columns
    reorderColumns() {
    ...
    // Helper methods for determining insertion points during drag & drop
    ...
    setupEventListeners() {
        ...
        this.kanbanContainer.addEventListener('drop', e => {
            ...
            if (draggedData.startsWith('column-')) return; // ignore column drops here
